...
namespace ofxTParams {
  ...
  , public ofxTCommon::JsonWritable {
  public:
    using ParamTagSet = _impl::ParamTagSet;
...
//    void linkGui(ofxGuiElement* element);
  ...
  namespace _impl {
    ...
    public:
      ...
      SelfT& withTag(Arg tag) {
        tags().enable(tag);
        return selfRef();
      }

      template<typename Arg>
      SelfT& withoutTag(Arg tag) {
        tags().enable(tag);
        return selfRef();
      }
    protected:
      template<typename Arg>
      SelfT& withTagSetTo(Arg tag, bool value) {
        tags().put(tag, value);
        return selfRef();
      }
      
      virtual SelfT& selfRef() = 0;
    };
...
//      }
  ...
  template<typename T>
  class TParam
  : public _impl::TTypedParamBase<T, TParam<T>> {
  public:

    void setNormalizedValue(float normVal) {
      auto val = ofxTCommon::getInterpolated(ofParameter<T>::getMin(),
                                             ofParameter<T>::getMax(),
                                             normVal);
      ofParameter<T>::set(val);
    }
    ...
    const char* getTypeName() const override {
      return _impl::TTypedParamBase<T, TParam<T>>::getTypeInfo().name();
    }
  ...
  template<>
  class TParam<bool>
  : public _impl::TTypedParamBase<bool, TParam<bool>> {
  public:
  ...
  , public _impl::TParamBaseWithInitializers<Params>
  ...
  public:
    ...
    using BaseT = _impl::TParamBaseWithInitializers<Params>;
